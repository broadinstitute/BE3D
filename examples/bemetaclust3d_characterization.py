"""
Step 3b (meta): Characterization of meta-aggregated screens.
Usage example: python beclust3d_characterization_meta.py ./yaml/dnmt3a_local.yaml

Requires Steps 1, 2, and 3a (bemetaclust3d_metaaggregate.py) to have been run first.
Only runs when the number of screens > 1.
"""
...
def main(**kwargs):
    ...
    if len(screen_names) <= 1:
        print("Only 1 screen present — meta characterization requires >1 screen. Exiting.")
        sys.exit()
    ...
    for score_type in ['LFC', 'LFC3D']:
        ...
        input_df2 = pd.concat([df_meta[colnames], df_struc, df_domains['Domain']], axis=1)
    ...
    df_dis_input['Meta_LFC']      = df_lfc_dis[f'{function_for_meta}_LFC']
    df_dis_input['Meta_LFC3D']    = df_lfc3d_dis[f'{function_for_meta}_LFC3D']
    df_dis_input['Meta_LFC3D_dis'] = df_lfc3d_dis[f'{function_for_meta}_LFC3D_dis']
    ...
    df_dis_input = df_dis_input.rename(columns={
        f'{function_for_meta}_LFC3D_neg_{cutoff_str}_psig': 'Meta_LFC3D_neg_psig',
        f'{function_for_meta}_LFC3D_pos_{cutoff_str}_psig': 'Meta_LFC3D_pos_psig',
    })

    lfc_lfc3d_scatter(
        df_input=df_dis_input, workdir=output_dir,
        input_gene=input_gene, screen_name='Meta', pthr=cutoff,
    )
    ...
    df_input = df_input[df_input[f'{function_for_meta}_{score_type}_wght'] != '-']
    df_input[f'{score_type}_wght'] = df_input[f'{function_for_meta}_{score_type}_wght'].abs() * 100
    df_input['direction'] = np.where(
        df_meta_wght[f'{function_for_meta}_{score_type}_wght'].astype(float) > 0, 'POS',
        np.where(df_meta_wght[f'{function_for_meta}_{score_type}_wght'].astype(float) < 0, 'NEG', 'ZERO')
    )
    ...
    g2p_formatted_hit_cluster(
        ...
        lfc_pthr=single_screen_pthr_str.split('.')[1],
        lfc3d_pthr=single_screen_pthr_str.split('.')[1],
        meta_pthr=multi_screen_pthr_str.split('.')[1],
    ...
    print("Step 3b complete: meta characterization done")
